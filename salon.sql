...
MAIN_MENU () {
  ...
  then 
    ...
    read CUSTOMER_PHONE
    OLD_CUSTOMER=$($PSQL "SELECT customer_id FROM customers WHERE phone='$CUSTOMER_PHONE'")
    if [[ -z $OLD_CUSTOMER ]]
    then
      ...
      # SERV_ID=$($PSQL "SELECT service_id FROM services WHERE name='$SERVICE_NAME'")
      INSERTED_APPOINTMENT_RESULT=$($PSQL "INSERT INTO appointments(customer_id,service_id,time) VALUES($CUST_ID,$SERVICE_ID_SELECTED,'$SERVICE_TIME')")
